...
/**
 * 콜백 함수란?
    콜백 함수는 다른 함수에 인수로 전달되어 특정 이벤트나 조건이 발생할 때 호출되는 함수입니다. React에서 이벤트 핸들러로 사용하는 함수는 대표적인 콜백 함수의 예입니다.
 * 
 ...
 * 1. deleteAlpha(value.id)의 의미
    ...
    // 삭제 로직일 뿐, 반환값이 없기 때문에 undefined일 확률이 높음. // 심지어 그 반환값이 onClick 설정됨. // onClick이 발생해도 아무런 효과가 없을 거임.
 ...
 * 2. ()=>deleteAlpha(value.id)의 의미
    해석: 이 표현은 deleteAlpha(value.id)라는 함수 호출을 나중에 실행하도록 준비해둔 "함수 참조"입니다. 이 화살표 함수는 "콜백 함수" 역할을 합니다.
 ...
 * 3. deleteAlpha 함수 호출에서 인자 없이 사용하면?
    해석: 만약 deleteAlpha를 인자 없이 호출하려면 그냥 deleteAlpha라고만 작성할 수 있습니다. 이 경우 함수는 참조로서 전달됩니다.
