...
const counterReducer = (state = initialState, action) => {
    switch (action.type) {
        case PLUS:
            return { number: state.number + 1 + action.payload }; // number 값을 1 증가. // payload 만큼 증가
        case MINUS:
            return { number: state.number - 1 - action.payload }; // number 값을 1 감소. // payload 만큼 감소
        default:
